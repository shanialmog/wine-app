...
class WineItem extends Component {
  ...
  isAllFieldsValid () {
    const costValid = !!/^\d+$/.test(this.state.cost)
    const rateValid = !!/^\d+$/.test(this.state.rate)
    const vintageValid = !!/^\d+$/.test(this.state.vintage)
    console.log(costValid, rateValid, vintageValid)
    return (!(vintageValid && costValid && rateValid))
  }
          ...
          <Box display='flex'>
            ...
            <Box width={50} pr={2.5}>
              <TextField
                ...
                margin='normal'
                error={!/^\d+$/.test(this.state.vintage)}
              />
            </Box>
